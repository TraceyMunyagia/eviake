-- Allow the public Evia Web quote form to create a client and quote order
-- without exposing general insert access to the admin tables.
...
begin
  if nullif(trim(p_name), '') is null
    ...
    or nullif(trim(p_pages), '') is null then
  ...
  values (
    ...
    lower(trim(p_email)),
    trim(p_phone),
    format('Category: %s\nDescription: %s\nPages: %s', trim(p_category), trim(p_description), trim(p_pages))
  )
  ...
  values (
    ...
    'quote',
    v_package,
    format('Quote submitted from eviaweb.com. Category: %s\nDescription: %s\nPages: %s', trim(p_category), trim(p_description), trim(p_pages))
  )
